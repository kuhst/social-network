...
import { actionsUsersReducer, FilterType, follow, responseUsers, unfollow } from '../../redux/UsersReducer'
import {
	getCurrentPage,
...
const UsersPage: React.FC<PropsType> = React.memo((props) => {
	...
	useEffect(() => {
		...
		dispatch(responseUsers(usersCountOnPage, actualPage, actualFilter, false))
		return () => {
	...
	const onPageChanged = (pageNumber: number, pageSize?: number) => {
		dispatch(responseUsers(pageSize || usersCountOnPage, pageNumber, filter, false))
	}
	...
	let usersItems = users.map((u) => (
		<User key={u.id} user={u} followingInProgress={followingInProgress} unfollow={unfollowUser} follow={followUser} />
	))

	return (
		<div>
			<UsersSearchForm isFetching={isFetching} onFilterChanged={onFilterChanged} />
